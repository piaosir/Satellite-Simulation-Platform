...
export function projectGrid(set, igrid, basis, box = null, out = null) {
  const { NX, NY } = set
...
export function bandGeometry(field, levelsAsc, wantFills = true, box = null) {
  ...
  const emitTri = (i0, i1, i2) => {
    ...
    let base = _bgBase, baseLen = 3
    if (_bgBase[3] < 0 || _bgBase[7] < 0 || _bgBase[11] < 0) {         // 沿 0°仰角线(m≥0)裁
      baseLen = clip(_bgBase, 3, 3, 0, true, _bgVis); base = _bgVis
      if (baseLen < 3) return
    }
    if (wantFills) {
      for (let k = 0; k < nb; k++) {
        let len = clip(base, baseLen, 2, levelsAsc[k], true, _bgP)     // d ≥ Lk
        let cur = _bgP
        if (k < nb - 1 && len) { len = clip(_bgP, len, 2, levelsAsc[k + 1], false, _bgQ); cur = _bgQ }  // 且 d ≤ Lk+1（顶档不封顶）
        if (len >= 3) { const poly = new Array(len); for (let i = 0; i < len; i++) poly[i] = [cur[i * 4], cur[i * 4 + 1]]; fills[k].push(poly) }
      }
    }
    for (let k = 0; k < nb; k++) {                                     // 各档等值线 = base 上 d==Lk 的穿越段
      ...
      for (let i = 0; i < baseLen; i++) {
        const ai = i * 4, bi = ((i + 1) % baseLen) * 4
        const da = base[ai + 2], dbb = base[bi + 2]
        if ((da < L) !== (dbb < L)) {
          const s = (L - da) / (dbb - da)
          const x = base[ai] + (base[bi] - base[ai]) * s, y = base[ai + 1] + (base[bi + 1] - base[ai + 1]) * s
