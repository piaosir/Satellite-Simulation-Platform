  ...
  const emitTri = (i0, i1, i2) => {
    ...
    if (wantFills) {
      ...
      if (fbLen >= 3) for (let k = 0; k < nb; k++) {
        ...
        if (len < 3) continue
        if (fillHull) { const poly = clipToHull(cur, len, hull); if (poly && poly.length >= 3) fills[k].push(poly) }
        else { const poly = new Array(len); for (let i = 0; i < len; i++) poly[i] = [cur[i * 4], cur[i * 4 + 1]]; fills[k].push(poly) }
      }
    }
    if (lineLen >= 3) for (let k = 0; k < nb; k++) {                   // 各档等值线 = lineBase 上 d==Lk 的穿越段
      const L = levelsAsc[k]
