...
const props = defineProps({
  ...
  // 冻结「关键列」（首列起到城市列的粘性左固定列）。默认 true 保持旧行为；链路表传 false 取消冻结、全列随横滚。
...
const frozenCount = computed(() => (props.freezeKeys ? (cityFieldIdx.value >= 0 ? cityFieldIdx.value + 1 : 1) : 0))
    ...
    <div v-if="imp.open" class="sg-mask sg-mask-lite" @click="closeImport">
      <div class="sg-box sg-imppop" :style="imp.style" @click.stop>
        ...
        <div v-if="impOtherGroup" class="sg-oth">
          ...
          <span v-else class="sg-oth-def" title="未指定另一端：新行对侧用默认站址。悬停列表条目点 ⇄ 可把该站设为另一端">默认（{{ impOtherDefault }}）· 列表悬停 ⇄ 可指定</span>
        </div>
