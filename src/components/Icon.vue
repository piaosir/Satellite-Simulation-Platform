...
// 用法：<Icon name="check" :size="16" />；颜色跟随 currentColor，全软件不再使用 ▾✓▤ 等文本字符做图标。
...
const ICONS = {
  ...
  // 侧栏「实时气象」入口：云 + LIVE 字样。与「环境场」的 cloud-rain 同一朵云（两者是同一类图，
  // 一张气候统计、一张实时预报），差别改由字样承担，比原来那个右上角圆点分得开得多。
  // ★ 雨线让位给字：24 格的画布里，云要 ~13 格才像云，字要 ~6 格才认得出，两样加起来已经占满 ——
  //   再塞三条雨线，只能把两边都压到看不清。字本身就说明了「这是天气」，雨线不必再说一遍。
  // ★ 字用与云同一档笔宽（不另设 stroke-width）：混两种笔宽在 18px 下会散成两件东西；
  //   LIVE 角标在广播惯例里本来就是粗体，这一档正好。
  // ★ 字形按几何排版，不靠目测：云的原始包围盒 x[2,22] y[3,16.24]，等比缩到 y[1.5,13] 后
  //   （s=0.8686）让出下半，字的大写高 6.2 格、落在 y[16.4,22.6]，与云底留 1.7 格（圆头笔帽
  //   两边各吃掉 0.85 格，不留就粘在一起）。
  'cloud-rain-live': '<path d="M5.05 11.84A6.08 6.08 0 1 1 15.22 5.84h1.55a3.91 3.91 0 0 1 2.17 7.16"/><path d="M3.8 16.4v6.2h3.1"/><path d="M8.6 16.4v6.2"/><path d="M10.4 16.4 12.5 22.6 14.6 16.4"/><path d="M20.2 16.4h-3.8v6.2h3.8"/><path d="M16.4 19.5h3.2"/>',
  ...
  // 行尾「更多」菜单（图层面板每行一颗）与「查找」漏斗（搜索框旁）
  ...
  // 抓取（手握）：装配页「拾起」——与「插座」的 locate-fixed、视口插座标记（圈 + 点）分得开
  ...
  // 方框 + 三根柱（Material Icons Outlined「insert_chart」，Apache-2.0 License，
  // github.com/google/material-design-icons · src/editor/insert_chart/materialiconsoutlined/24px.svg）：
  // 空间态势报告入口用 —— 这个模块交付的是一份统计报告（屏上可翻、可导出 Word），不是一台观测设备，
  // 故图标画「统计」不画雷达。
  // ★ 原文是【填充】轮廓路径（Material 全集如此），这里按同样的几何转写成描边：
  //   外框 3↔21 的 2px 边 → 笔心落在 4 的 16×16 圆角方（stroke-width 2 覆盖回 [3,5]）；
  //   三根柱 x 中心 8 / 12 / 16 与原文逐格对齐，只把两端各收 1 格 —— 本表用圆头笔帽，
  //   不收的话柱头会各鼓出 1 格顶到框内边（对照 C 档实测）。转写成描边而不是照抄填充路径的理由：
  //   全表 30 余枚都是描边，填充件不跟 Icon.vue 的 stroke-width 属性走（视图切换那两枚就传了 1.7）。
  // ★ 与工具栏左右邻居 folder-open（文件夹）、freq-plan（方框中一个 F）：三者都有外框，靠框内
  //   「三根不等高的柱」vs「字母 F」vs「无框折角」区分，15px 下实测可分（见 .ssaharness 的并排样张）。
  // ★ 与 table（方框 + 十字分隔，PFD 掩模）同为方框系：那枚是通到边的十字线、这枚是不到边的三根竖柱，
  //   且两者中间隔着 5 枚图标与 1 条分隔线。日后若把工具栏重排到两者相邻，这枚须重选。
  'insert-chart': '<rect x="4" y="4" width="16" height="16" rx="2"/><path d="M8 16v-5"/><path d="M12 16V8"/><path d="M16 16v-2"/>',
  ...
  // 装配设计器（吸附 / 镜像）与实体模型（飞机 / 船 / 车）的图标（Lucide，ISC License）
