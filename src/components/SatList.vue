...
//
// 选择口径（对齐 Excel / 资源管理器）
//   点选=只选它 · Ctrl 点=增减 · Shift 点=从锚点连选 · Ctrl+Shift 点=把连选并入 · 左键拖=刷选
...
// 换数据源（展开另一处 / 组成员变了）→ 清选择、回到顶部
watch(() => props.items, () => { kw.value = ''; kwLive.value = ''; clearSel(); scrollTop.value = 0; if (vpEl.value) vpEl.value.scrollTop = 0 })
...
const AUTO_MS = 16
let dragging = false, dragBase = null, dragY = 0, dragIdx = -1
...
function beginDrag(e, i, base) {
  dragging = true; dragBase = base; dragIdx = i; dragY = e.clientY
...
function onRowDown(e, idx) {
  ...
  if (e.shiftKey) selectRange(i, ctrl ? sel.value : null)
  else if (ctrl) { toggleAt(i); beginDrag(e, i, new Map(sel.value)) }   // Ctrl 拖＝在已选之上加刷
  else { selectOnly(i); beginDrag(e, i, null) }
  ...
  <div ref="rootEl" class="sl" :class="{ hassel: count > 0 }">
    ...
    <div class="slm">
      ...
      <span
        ...
        :title="(!a.always && !count) ? '先在下面选中卫星：点选一颗 · Ctrl 点加减 · Shift 点连选 · Ctrl+A 全选' : (a.title || a.label)"
        @click="onAction(a, $event)"
      ><Icon v-if="a.icon" :name="a.icon" :size="11" />{{ a.label }}</span>
    </div>
...
.slf:focus-within { border-color: var(--accent); }
.slf input { flex: 1; min-width: 0; border: 0; background: transparent; color: var(--text); font: inherit; font-size: 11.5px; padding: 2px 0; outline: none; }
...
.slvp:focus-visible { box-shadow: inset 0 0 0 1px var(--accent); }
...
.slmsg { padding: 3px 12px 7px; font-size: 11px; color: var(--text-faint); }
