<script setup>
// 「高级计算」对话框：多载波功带平衡（VSAT 组网），GEO 窗专用。
//
// 它解决的是单链路口径解决不了的事——转发器上跑一组载波时，平衡是整组的账：前向 TDM 超发、
// 返向 TDMA 欠发，各自都不平衡，合起来 Σ功率带宽 = Σ载波带宽 才是要的结果。求解在核心算法
// 外层（见 shared/advBalance.js），结果落成各载波的「设置余量」，落地后照常走一次正常计算，
// 屏幕上的数全部来自引擎本身。
//
...
import { ADV_BASES, solveAdv } from '../shared/advBalance.js'
...
const res = computed(() => solveAdv({
  picked: picked.value, state: cstate, base: base.value, overDb: overDb.value,
  tpBwMHz: props.tpBwMhz,
  target: target.value, targetBwMHz: targetBwMHz.value,
...
const totalUnit = computed(() => {
  const r = res.value
  return pickBw(r.ok ? [r.occBwKHz, r.targetKHz, r.beforePbwKHz, r.afterPbwKHz] : [], false)
})
...
function apply() {
  ...
  emit('apply', {
    base: base.value, overDb: parseFloat(overDb.value) || 0,
    ...
    <div class="ab" role="dialog" aria-modal="true">
      ...
      <div class="ab-bd">
        ...
        <div class="ab-tw">
          <table class="ab-t">
            ...
            <tbody>
              ...
              <tr v-if="!rows.length"><td colspan="8" class="ab-empty">链路表暂无计算结果。</td></tr>
            </tbody>
          </table>
        </div>
        ...
        <div v-else class="ab-out">
          <div class="ab-kv" title="本组载波在转发器上占用的频谱带宽：各载波各占一段频谱，求和"><span>组占用带宽</span><b>{{ tot(res.occBwKHz) }}</b><i>{{ totU }}</i></div>
          ...
          <div v-if="isFinite(res.bwUseAllPct)" class="ab-kv" title="全表占用：链路表里所有算出结果的行，参与本组配平的按解后值、其余按其此刻的值。与上一行并列着看，才知道这只转发器还剩多少"><span>全表占用</span>
