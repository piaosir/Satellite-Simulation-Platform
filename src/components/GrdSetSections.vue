...
// 起始 / 间隔随类型走：相对档从 −1 起、绝对档从「峰值取整 − 1」起（与 addLevel 的首档同口径）；换类型 / 换聚焦天线时重置
function genSync() {
  const m = grd.antMeta(), peak = m ? +m.peakDb : NaN
  gen.start = st.ctype === 'abs' ? (Number.isFinite(peak) ? Math.floor(peak) - 1 : 50) : -1
  gen.step = -1
}
watch(() => [st.ctype, grd.active.value], genSync, { immediate: true })
    ...
    <div class="sec">
      ...
      <template v-if="isSecOpen('grd-set')">
        ...
        <div class="glv">
          ...
          <div class="glvhd" aria-hidden="true"><span class="h-clr">填充</span><span class="h-clr">线</span><span class="h-val">值 (dB)</span><span class="h-nm">名称</span><span class="h-del"></span><span class="h-del"></span></div>
