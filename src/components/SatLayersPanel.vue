...
function openAdd(ev) {
  ...
  menu.value = { kind: 'add', row: null, items, ...place(items, r.left, r.bottom + 4) }
}
...
onBeforeUnmount(() => { clearTimeout(armT); endDrag() })
...
function onDragMove(ev) {
  ...
  if (b) {   // 贴边自动滚动
    ...
    if (ev.clientY < r.top + 24) b.scrollTop -= 8
    else if (ev.clientY > r.bottom - 24) b.scrollTop += 8
  }
  ...
  <div class="lp">
    ...
    <div ref="bodyEl" class="lp-body" :class="{ dragging: !!drag }">
      ...
      <template v-if="searchRow">
        <div
          class="lp-row pinned"
          :class="{ off: !searchRow.visible, exp: expanded === searchRow.id, solo: solo && solo.id === searchRow.id }"
        ...
        >
          ...
          <span class="lp-chev"><Icon :name="expanded === searchRow.id ? 'chevron-down' : 'chevron-right'" :size="12" /></span>
        </div>
        <SatList
      ...
      <template v-for="(row, idx) in rows" :key="row.id">
        ...
        <div
          ...
          :class="{ off: !row.visible, exp: expanded === row.id, solo: solo && solo.id === row.id, arm: delArm === row.id, dragsrc: drag && drag.id === row.id }"
    ...
    <div v-if="epoch !== null" class="lp-foot" title="全部自定义星座共用的场景历元：星座定向以此为准，拖时间轴仍从此历元向后推演">
      <label>场景历元</label>
...
.lp-ib.on { border-color: var(--accent); color: var(--accent); }
.lp-add { background: var(--accent); border-color: var(--accent); color: var(--bg); }
.lp-add:hover { color: var(--bg); filter: brightness(1.08); }
...
.lp-dropline { height: 2px; margin: -1px 8px; background: var(--accent); border-radius: 1px; }
...
.lp-row.off .lp-nm, .lp-row.off .lp-cnt, .lp-row.off .lp-sub, .lp-row.off .lp-swi, .lp-row.off .lp-ki { opacity: 0.45; }
.lp-row.solo .lp-nm { color: var(--accent); }
.lp-row.arm { box-shadow: inset 2px 0 0 var(--danger); }
.lp-row.dragsrc { opacity: 0.35; }
.lp-row.pinned { border-bottom: 1px dashed var(--border); margin-bottom: 3px; }
.lp-grip { flex: none; width: 14px; height: 18px; display: inline-flex; align-items: center; justify-content: center; color: var(--text-faint); opacity: 0; cursor: grab; touch-action: none; }
...
.lp-menu { position: fixed; z-index: 2200; width: 200px; max-height: 360px; overflow-y: auto; background: var(--surface); border: 1px solid var(--border-strong); box-shadow: var(--shadow-2); border-radius: var(--r-box); padding: 4px 0; }
...
.lp-mi.dis { opacity: 0.4; pointer-events: none; }
