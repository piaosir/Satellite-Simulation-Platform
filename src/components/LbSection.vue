...
defineProps({
  ...
  summary: { type: String, default: '' },      // 折叠态/常态都显示的一行摘要（mono）
...
<template>
  <section class="lbx-sec" :data-sec="id">
    <header class="lbx-sec-hd" :class="{ chain: flow === 'chain' }" @click="$emit('toggle')">
      <span class="lbx-sec-chev" :class="{ closed: collapsed }"><Icon name="chevron-down" :size="13" /></span>
      ...
      <!-- 节级动作（阻断冒泡，不触发折叠） -->
      <span class="lbx-sec-acts" @click.stop><slot name="actions" /></span>
    </header>
    <div v-show="!collapsed" class="lbx-sec-bd" :class="{ flush: $slots.default && flow === 'grid' }">
      <slot />
    </div>
  </section>
</template>
...
.lbx-sec-hd:hover .lbx-sec-t { color: var(--text); }
.lbx-sec-chev { display: inline-flex; align-items: center; color: var(--text-faint); transition: transform .12s ease; }
.lbx-sec-chev.closed { transform: rotate(-90deg); }
...
.lbx-sec-sp { flex: 1; }
.lbx-sec-acts { display: inline-flex; align-items: center; gap: 4px; cursor: default; }
