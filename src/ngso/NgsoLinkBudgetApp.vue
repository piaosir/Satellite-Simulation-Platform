...
import { SAT_FIELDS, CARRIER_FIELDS, TX_FIELDS, RX_FIELDS, ES_FIELDS, ES_COMMON_FIELDS, ES_TX_FIELDS, ES_RX_FIELDS, defaultsFor, buildParams } from './ngsoParams.js'
...
// —— 地球站配置库：每份配置 = 一种站型的收发射频参数（公共天线口径 + 发射链/接收链分列，字段见 ngsoParams.js station 组）。
...
async function refreshReadonly() {
  ...
  for (const tx of txStations) {
    try { const { satParams, linkParams } = buildParams(satForm, resolveBaseband(tx.basebandId).form, tx, rx0, resolveEs(tx.stationId).form, resolveEs(rx0.stationId).form); linkParams.margin = targetMarginDb.value; const r = await api.linkBudget.computeModeNGSO(satParams, linkParams, opt); if (r && r.success) te[tx._id] = fix2(r.data.stationEIRPResult) } catch (e) { /* skip */ }
  }
  for (const rx of rxStations) {
    try { const { satParams, linkParams } = buildParams(satForm, resolveBaseband(tx0.basebandId).form, tx0, rx, resolveEs(tx0.stationId).form, resolveEs(rx.stationId).form); linkParams.margin = targetMarginDb.value; const r = await api.linkBudget.computeModeNGSO(satParams, linkParams, gtOpt); if (r && r.success) rg[rx._id] = fix2(r.data.gOverTeResult) } catch (e) { /* skip */ }
  }
...
async function compute() {
  ...
  try {
    ...
    for (const [ti, ri] of pairs) {
      ...
      const { satParams, linkParams } = buildParams(satForm, bbForm, tx, rx, resolveEs(tx.stationId).form, resolveEs(rx.stationId).form)
  ...
  <div class="lb-shell">
    ...
    <div class="lb-body">
      ...
      <section class="lb-col lb-build">
        ...
        <!-- 选中模块的编辑器 -->
        <div class="lb-edit">
          <StationGrid v-if="activeModule === 'tx'" :stations="txStations" :fields="TX_FIELDS" :cities="cities" :city-search="citySearch" label="发信站" :auto-geo="autoGeoTx" ro-label="工作点EIRP" ro-unit="dBW" :ro-values="txEirp" :select-options="{ basebandId: basebandSelectOptions, stationId: esSelectOptions }" />
          <StationGrid v-else-if="activeModule === 'rx'" :stations="rxStations" :fields="RX_FIELDS" :cities="cities" :city-search="citySearch" label="收信站" :auto-geo="autoGeoRx" ro-label="工作点G/T" ro-unit="dB/K" :ro-values="rxGt" :select-options="{ stationId: esSelectOptions }" />
          ...
          <div v-else-if="activeModule === 'station'" class="bb-wrap es-wrap">
            ...
            <p class="bb-tip">每份地球站配置 = 一种站型：收发共用的天线口径 + 发射链 / 接收链参数（效率按频段分设，含干扰项；转发器互调 Tpdr. C/IM 属星上硬件，仍在「卫星」模块）。「发信站群 / 收信站群」表的「地球站配置」列为每行选择套用哪一份：发信站取发射参数、收信站取接收参数，同一份可被多行乃至收发两侧共用；站表只留地球站位置（经纬度/最低仰角等）与逐站配对量。</p>
            <div class="bb-cards">
