...
// 载波带宽。可真实转发器上跑的是【一组】载波，平衡是整组的事：VSAT 组网里前向 TDM 大载波按可用度
// 超发（功率占用 > 带宽占用）、返向 TDMA 小站受功放限制欠发（带宽占用 > 功率占用）；单看每条都不
// 平衡，合起来 Σ功率带宽 = Σ载波带宽 才是要的结果。
// （曾并列过一档「CNC 载波叠加」——两条链路共用一段频谱、功率叠加，连同引擎侧的「附加 C/I」入参与
// 链路表的「路数」列，2026-09-07 一并删除；老存档里带 advBalanceMode:'cnc' 标记的副本视同普通配置。）
//
...
export function validateAdv(picked) {
 ...
 * 组平衡求解。
 * @param {object} o
 *   picked   [{ no, rowId, carrierId, carrierName, bwKHz, pbwKHz, marginDb, baseDb, error, paW, paPresetW, booDb, boiDb }]
 *            已勾选的链路（带上一次计算结果）；baseDb＝该载波「进本功能之前」的原始余量（宿主用 advBaseMargin
 *            从载波配置上取，缺省即 marginDb）；paW / paPresetW＝发端功放此刻实算值与站型预设；booDb / boiDb＝卫星条目的回退
 *   state    { [carrierId]: { bias:Number } } 逐载波的偏置（相对基准的固定错位）
 *   base     'current' | 'balance'
 *   overDb   组级超发量（目标总功率带宽相对组占用带宽抬高的 dB 数，0 即严格平衡）
 *   target   'sum'（Σ载波带宽，缺省）| 'fixed'（指定带宽，取 targetBwMHz）
 *   tpBwMHz  转发器带宽（只用于占用率读数，缺省则不报占用率）
 *   allRows  全表行 [{ rowId, bwKHz, pbwKHz }]（可选；给了才出全表占用读数）
 */
export function solveAdv(o) {
  ...
  const msg = validateAdv(picked)
  if (msg) return fail(msg)

  const carriers = groupByCarrier(picked)
  // 组占用带宽：各载波各占一段频谱 → 求和
  const occBwKHz = picked.reduce((s, p) => s + p.bwKHz, 0)
  ...
  // balance = 令该载波自己的功率带宽等于自己的载波带宽的那个余量
  for (const c of carriers) {
    c.balanceDb = c.A > 0 ? dB(c.bwKHz / c.A) : NaN
  ...
  for (const c of carriers) {
    ...
    for (const p of c.links) {
      const after = normPbw(p.pbwKHz, p.marginDb) * lin(c.toDb)
      afterPbwKHz += after
  ...
  if (picked.length >= 2) {
    ...
    if ((isFinite(boo) && boo < 2) || (isFinite(boi) && boi < 4)) {
      warnings.push(`多载波组（${picked.length} 路）· 转发器 OBO ${isFinite(boo) ? boo.toFixed(1) : '—'} dB / `
  ...
  return {
    ok: true, message: '', warnings, base, overDb, deltaDb,
    occBwKHz, targetKHz, beforePbwKHz, afterPbwKHz,
...
function allUse(allRows, outLinks, tpBwKHz) {
  ...
  for (const r of allRows) {
    ...
    const b = hit ? hit.bwKHz : num(r.bwKHz)
    const p = hit ? hit.pbwAfter : num(r.pbwKHz)
...
// —— 写回落点：解出的余量该写进哪份载波配置 ——
//
// 组网平衡的结论是【这一组链路在这套工况下】的，不是载波自身的属性：同一份载波换一组链路配平
// 就是另一个余量，一份配置装不下多个结果。故一律派生专用副本，用户原来的载波配置一字不动。
// 反复试错不生冗余：勾选行此刻指着的若已是本功能派生的副本（form 上带 ADV_MARK 标记），
// 就地更新那一份而不是再复制一层。
export const ADV_MARK = 'advBalanceMode'     // 副本标记（＝「这份是高级计算的副本」）
...
const ADV_MODE = 'vsat'                      // ADV_MARK 的取值（历史上还有过 'cnc'，那些副本已不再认作自己的）
const ADV_SUFFIX = ' · VSAT平衡'
...
// （老存档里可能还留着已删那档的「· CNC平衡」后缀，一并剥掉）
const stripSuffix = (s) => String(s == null ? '' : s).replace(/ · (VSAT|CNC)平衡( \d+)?$/, '')

/**
 * 规划写回（纯函数，什么都不改——只回一份「该怎么改」的清单，由窗口照单执行）。
 * @param {object} o
...
export function planAdvWriteback(o) {
  ...
  for (const c of (o.carriers || [])) {
    ...
    const mine = form[ADV_MARK] === ADV_MODE   // 本功能派生的副本＝我们自己的东西，可以直接改
    if (!usedByOthers && mine) {
    ...
    let name = (stripSuffix(root.name) || '载波') + ADV_SUFFIX
    ...
    ops.push({
      ...
      formPatch: { ...patch, [ADV_MARK]: ADV_MODE, [ADV_ORIGIN]: rootId }
    })
  }
  return { ops }
}
