 ...
 * 口径与 linkCalculator.js 同名函数一致，改一处两边同改。
 * @param {number} n 数值
...
function performCalculations(satParams, inputs) {
  ...
  // 仰角取与主链路雨衰同一个 PropElev（§8 开启时即等效仰角），保证同源。
  const uplinkRainXPD = calculateRainXPD_P618_14(
    ...
    { R001: rainRate, pol: uplinkPolarization, lat: earthLat, lon: earthLon, orbitPos: orbitPosition, altitude, elevOverride: uplinkPropElev }
  );
  const downlinkRainXPD = calculateRainXPD_P618_14(
    ...
    { R001: rxRainRate, pol: downlinkPolarization, lat: rxLatitude, lon: rxLongitude, orbitPos: orbitPosition, altitude: rxAltitude, elevOverride: downlinkPropElev }
  );
  ...
  // 不再由反解 downlinkCN 取残差——上行主导 + UPC 未全补偿时残差含上行残余雨衰（NGSO 还含 ISL 份额），
  // 会把本行低报数 dB。
  ...
  // 平台三窗从来没有它的输入框 —— 引擎只能靠空值回退硬造一个角度，于是这几项读数全是
  // 「基于一个凭空数字」的假合规量（NGSO 更无「静止弧邻星」可言）。故整组删除。
  ...
  // ISL 段总距离（km）。走 ISL 时上行+下行两段时延加不出总时延，SLA 的「单程时延」依据列
  // 靠这一项把三段距离摆齐：(d_up + d_ISL + d_down) / c。0 跳时为 0，依据列会把它跳过。
  results.islDistanceResult = islTotalDistance.toFixed(2 + FX);
  ...
  // 信息速率两个来源：DVB 行是表单里那个数，3GPP NTN 行由 TBS ÷ 时长算出来（常是无限小数）。
  // 收到 3 位并去尾零，口径同 GEO 引擎；再生式与端到端都取这份出参，故三个窗一起治。
 ...
 * 与 GEO 引擎 linkCalculator.js 的同名函数逐字一致（两份副本不许漂）。
 *
