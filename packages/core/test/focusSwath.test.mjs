...
import { headingAz, swathK, swathSig, sectionOf } from '../../../src/viz/constellation/focusSwath.js'
import { llaToVec, vecToLatLon, swathFill, swathEdges, createSink, FILL_R, FILL_CELL } from '../../../src/viz/globe3d/focusLanes.js'
...
function edgeCheck(mode, tag) {
  ...
  ok(angDeg(c, mid) < 0.2, `${tag} 横断面中点贴近星下点`, `${angDeg(c, mid).toFixed(3)}°`)
...
// —— ④ 地面航向 ——
{
  ...
  const azE = headingAz(pe, g) / DEG, azP = headingAz(pp, g) / DEG
...
// —— ⑤ 三角网 ——
{
  ...
  ok(L.length === 3 && R.length === 3 && Math.abs(L[0].length() - 1 - 12 / 6371) < 1e-6, '⑤ 两缘点列抬到 LIFT')
  // 断面缺失 → 该格跳过不抛
