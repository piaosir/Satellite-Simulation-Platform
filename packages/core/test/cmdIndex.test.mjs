...
ok('宇宙空间 → 夜区颜色 / 星空亮度 / 晨昏线（复选项带 title 说明）', has('geo-space', '夜区颜色') && has('geo-space', '星空亮度') && by('geo-space')[0].items.some((i) => i.label === '晨昏线' && i.hint))
ok('复选项也入索引（显示波束名）', has('gxt-disp', '显示波束名'))
ok('行级 title 作说明', by('mk-points')[0].items.some((i) => i.label === '描边' && i.hint))
ok('宇宙空间归地图设置视图（原晨昏线分区 geo-term 已并入）', by('geo-space')[0] && by('geo-space')[0].view === 'geo' && !by('geo-term').length)
