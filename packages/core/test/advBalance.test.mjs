// 高级计算「多载波组功带平衡」（VSAT 组网）测试。运行：npm test
//
// 关键不变式：
...
for (const [tag, opt] of [
  ...
  const res = solveAdv({ picked: base, tpBwMHz: 36, ...opt })
...
for (const b of ['current', 'balance']) {
  const r1 = solveAdv({ picked: base, state: {}, base: b, overDb: 0 })
  const settled = measure(SET, (x) => r1.carriers.find((c) => c.id === x.carrierId).toDb)
  const r2 = solveAdv({ picked: settled, state: {}, base: b, overDb: 0 })
...
// 保护带与载波间隔留白由此进账：租 9 MHz 而载波只占 8.2 MHz 时，功率该按 9 MHz 配。
{
  ...
  const r = solveAdv({ picked: one, state: {}, base: 'current', overDb: 0, tpBwMHz: 36, target: 'fixed', targetBwMHz: 9 })
  ...
  const r2 = solveAdv({ picked: one, state: {}, base: 'current', overDb: 1.5, tpBwMHz: 36, target: 'fixed', targetBwMHz: 9 })
  near('⑯ 指定带宽 + 组超发 1.5 dB：Σ功率带宽 = 9 MHz × 10^(1.5/10)',
    r2.ok ? r2.afterPbwKHz : NaN, 9000 * Math.pow(10, 0.15), 1e-6)
  ok('⑯ 指定带宽为 0 / 空 → 拦下而不是算成 0',
    !solveAdv({ picked: one, state: {}, base: 'current', overDb: 0, target: 'fixed', targetBwMHz: 0 }).ok
    && !solveAdv({ picked: one, state: {}, base: 'current', overDb: 0, target: 'fixed', targetBwMHz: '' }).ok)
...
// —— ⑰ 三条组网告警：功放超预设 / 多载波回退 / 全表占用 ——
{
  ...
  const r = solveAdv({ picked: withPa, state: {}, base: 'current', overDb: 6, tpBwMHz: 36 })
  ...
  const rOk = solveAdv({ picked: one.map((p) => ({ ...p, paW: 20, paPresetW: 400 })), state: {}, base: 'current', overDb: 0, tpBwMHz: 36 })
  ok('⑰ 功放够用则不告警', rOk.ok && !rOk.warnings.some((w) => w.includes('功放需')))
  // 转发器回退：多载波组而卫星条目上填的是单载波回退
  const sc = one.map((p) => ({ ...p, booDb: 0.5, boiDb: 1 }))
  ok('⑰ 多载波组 + 单载波回退 → 告警',
    solveAdv({ picked: sc, state: {}, base: 'current', overDb: 0, tpBwMHz: 36 }).warnings.some((w) => w.includes('多载波组')))
  ok('⑰ 多载波回退够深则不告警',
    !solveAdv({ picked: one.map((p) => ({ ...p, booDb: 3, boiDb: 6 })), state: {}, base: 'current', overDb: 0, tpBwMHz: 36 })
      .warnings.some((w) => w.includes('多载波组')))
  ok('⑰ 卫星条目没给回退则不判', !solveAdv({ picked: one, state: {}, base: 'current', overDb: 0, tpBwMHz: 36 })
    .warnings.some((w) => w.includes('多载波组')))
  // 全表占用：表里还有两条没参与配平的行
  const others = [
    { ...one[0], rowId: 'x1', no: 90 },
    { ...one[1], rowId: 'x2', no: 91 }
  ]
  const rAll = solveAdv({ picked: one, state: {}, base: 'current', overDb: 0, tpBwMHz: 36, allRows: [...one, ...others] })
  ...
  ok('⑰ 全表占用把未勾选行也算进去', rAll.ok && rAll.allRowsN === 6)
  ok('⑰ 宿主没给 allRows 时不出这个读数（不编数）',
    !('bwUseAllPct' in solveAdv({ picked: one, state: {}, base: 'current', overDb: 0, tpBwMHz: 36 })))
}

// —— ⑤ 拦截与不可解 ——
ok('拦下空选择', !!validateAdv([]))
ok('拦下无结果的行', !!validateAdv([{ no: 1, bwKHz: NaN, pbwKHz: NaN, marginDb: NaN, error: '卫星不可见' }]))
const noP = base.map((p) => ({ ...p, pbwKHz: 0 }))   // 带宽还在、功率带宽为 0：过得了校验，解不出来
const zeroBal = solveAdv({ picked: noP, state: {}, base: 'balance', overDb: 0 })
ok('平衡点基准算不出 → 不可解', !zeroBal.ok)
ok('不可解仍带回载波清单（界面上的偏置输入才有地方改）', zeroBal.carriers.length === 2)
ok('功率带宽全为 0 → 不可解', !solveAdv({ picked: noP, state: {}, base: 'current', overDb: 0 }).ok)

// —— 兜底：脏输入不炸 ——
ok('缺参不炸', (() => { try { return solveAdv({}).ok === false } catch (e) { return false } })())
ok('负余量出警告', (() => {
  const r = solveAdv({ picked: base, state: { cFwd: { bias: 12 } }, base: 'current', overDb: 0 })
  return r.ok && r.warnings.some((w) => w.includes('负余量'))
})())

// —— ⑥ 写回落点 planAdvWriteback ——
// 不变式：VSAT 永不动用户原来的载波配置（组网平衡是「这组链路在这套工况下」的结论，不是载波自身的
// 属性，一份配置装不下多个结果）；反复配平复用同一份副本。
//
...
function runApply(lib, carriers, rowIds) {
  const { ops } = planAdvWriteback({ carriers, rowIds, rows: lib.rows, configs: lib.configs })
  const forks = []
  for (const op of ops) {
    if (op.kind === 'fork') {
...
{ // VSAT 全勾选（无外部引用）也照样新建：原配置一字不动
  ...
  const { forks } = runApply(lib, PLAN, ALL)
  ...
  const r2 = runApply(lib, [{ id: forks[0].id, toDb: 5.75 }, { id: forks[1].id, toDb: 0.5 }], ALL)
...
{ // 副本又被本表中未勾选的链路引用 → 再派生一份，名字按【根配置】重起、自动加序号
  const lib = makeLib()
  runApply(lib, PLAN, ALL)
  const rtn = lib.rows.find((r) => r.rowId === 'r2').carrierId
  const { forks } = runApply(lib, [{ id: rtn, toDb: 2.5 }], ['r2'])   // 只勾 r2，r3/r4 还引用着
...
{ // 没解出值的载波不写回
  const lib = makeLib()
  const { ops } = runApply(lib, [{ id: 'bb1', toDb: NaN }, { id: 'bb2', toDb: 1 }], ALL)
...
function makeAdvHost() {
  ...
  function round(bias) {
    ...
    const res = solveAdv({ picked, state, base: 'current', overDb: 0 })
    ...
    const { ops } = planAdvWriteback({ carriers, rowIds: rows.map((r) => r.rowId), rows, configs })
    for (const op of ops) {
      if (op.kind === 'fork') {
...
}
{
  ...
  const t = h.totals()
  // 容差＝写回量化：余量落库只留 3 位小数（±0.0005 dB ≈ 1.15e-4 相对），2e-5 那档只是碰巧过；2026-09-16 可用度出厂改 100%
  // 后云衰归零、数落到量化格的另一侧（Σ 差 +0.6 kHz / 10931 kHz = 5.5e-5），故按量化上界放到 1.2e-4
  near('反复应用不漂：每一轮仍然是平的（引擎实测 Σ功率带宽 = Σ载波带宽）', t.p, t.b, Math.max(0.01, t.b * 1.2e-4))
